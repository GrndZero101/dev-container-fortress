...
from ft.installer import InstallPlan, install_tool
...
def test_plan_uses_environment_defaults(monkeypatch: object) -> None:
    ...
    result = runner.invoke(app, ["plan", "--tool", "tenv"])

    assert result.exit_code == 0
    assert "tenv plan" in result.stdout
    assert "4.8.3" in result.stdout
